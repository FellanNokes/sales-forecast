...
Generates realistic fake sales transactions for the coming N days,
modulated by weather forecast data fetched from the `weather_forecast`
Supabase table.

Weather categorization (temp_category, weather_condition) is handled by
add_weather_features() from app.weather.weather_features.
...
Usage:
    python generate_fake_sales.py --days 7
    python generate_fake_sales.py --days 14 --output data/fake_sales.csv
"""
...
PRODUCTS = [
    ...
    # Drinking Chocolate — boosted on cold/snowy days
...
# ---------------------------------------------------------------------------
# Weather → sales multipliers (derived from historical data analysis)
# ---------------------------------------------------------------------------
...
# Category boosts per temperature — cold drives hot drinks and bakery
...
# Hourly transaction weights — from real data distribution (06:00–20:00)
HOURLY_WEIGHTS = {
...
def fetch_weather_for_dates(start: date, end: date) -> pd.DataFrame:
    ...
    # Normalize store_location to short name e.g. "Lower Manhattan"
    # (strips ", New York, United States" suffix if present)
    df["store_location"] = (
        df["store_location"]
...
def _generate_transactions_for_day(
    ...
    for i, (pid, hour) in enumerate(zip(chosen_products, chosen_hours)):
        ...
        transactions.append({
            ...
            "weather_condition": forecast_row["weather_condition"],
            "temp_category": forecast_row["temp_category"],
            "temperature_mean": forecast_row["temperature_mean"],
            "is_synthetic": True,
        })
    return transactions
...
def generate_fake_sales(
    ...
    """
    Generate fake sales data for the coming N days, weather-adjusted.
    ...
    Returns:
        DataFrame matching the coffee-shop-sales-revenue.csv schema.
    """
    ...
    total_revenue = (df["transaction_qty"] * df["unit_price"]).sum()
    print(f"\nGenerated {len(df):,} transactions | Total revenue: ${total_revenue:,.2f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate weather-adjusted fake coffee shop sales.")
